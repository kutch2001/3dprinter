...
difference ()
{
	rotate ([0,0,100]) union ()
		...
		translate ([0,0,0]) intersection ()
			{
			rotate ([0,0,90]) cylinder (h=boxwidth/2, r1 = (boxwidth/2)-smaller, r2 = (boxwidth/2)+(smaller/2));
			translate ([0,0,boxheight/4]) octagon (boxwidth, boxheight/2);
			}
		translate ([0,0,boxheight/(1+(1/3))]) octagon (boxwidth, boxheight/2);
...
//translate ([50,0,0]) cube ([1,2,boxheight+connectorheight]); //used to check measurements
